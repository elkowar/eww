            ...
            if let Ok(attr_map) = attr_map {
                $args.eww_state.resolve(
                    ...
                    //})
                    {
                        ...
                        move |attrs| {
                            $(
                                let $attr_name = attrs.get( ::std::stringify!($attr_name) ).with_context(|| format!("Error getting attribute {}", ::std::stringify!($attr_name)))?.$typecast_func()?;
...
>>>>>>> 638cb9d (Boilerplate for the new widget)
