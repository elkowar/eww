...
macro_rules! def_widget {
    ($args:ident, $scope_graph:ident, $gtk_widget:ident, {
        ...
        $({
            ...
            if let Ok(attr_map) = attr_map {
                let required_vars = attr_map.values().flat_map(|expr| expr.as_ref().map(|x| x.collect_var_refs()).unwrap_or_default()).collect();
    ...
    (@get_value $args:ident, $name:expr, = $default:expr) => {
        Some($args.widget_use.attrs.ast_optional::<simplexpr::SimplExpr>($name)?.clone().unwrap_or(simplexpr::SimplExpr::synth_literal($default)))
    };
