...
fn build_gtk_event_box(bargs: &mut BuilderArgs) -> Result<gtk::EventBox> {
    ...
    def_widget!(bargs, _g, gtk_widget, {
        ...
        prop(
            ...
            parse_keypress(gtk_widget.as_ref(), timeout, keypress);
        },
    });
    Ok(gtk_widget)
}
...
fn parse_keypress(gtk_widget: &gtk::Widget, timeout: Duration, keypress: String) -> () {
...
/// Connect a keypress to a gtk widget allowing to execute a given command
